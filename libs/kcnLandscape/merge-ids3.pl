...
print $cmd;
system $cmd;
$cmd = sprintf "awk '{ printf \"%%s\\tU0\\n\", \$1 }' %s/ids/%s_ecv_4_mappable_unique_noerror.ids >> %s/classify/tmp_class.txt\n",
        $idsPath, $PrjPrefix, $idsPath;
print $cmd;
system $cmd;
$cmd = sprintf "awk '{ printf \"%%s\\tUs\\n\", \$1 }' %s/ids/%s_ecv_5_mappable_unique_subonly.ids >> %s/classify/tmp_class.txt\n",
        $idsPath, $PrjPrefix, $idsPath;
print $cmd;
system $cmd;
$cmd = sprintf "awk '{ printf \"%%s\\tUx\\n\", \$1 }' %s/ids/%s_ecv_6_mappable_unique_others.ids >> %s/classify/tmp_class.txt\n",
        $idsPath, $PrjPrefix, $idsPath;
print $cmd;
system $cmd;

$cmd = sprintf "awk '{ printf \"%%s\\tV\\n\", \$1 }' %s/ids/%s_ecv_7_mappable_special.ids >> %s/classify/tmp_class.txt\n",
        $idsPath, $PrjPrefix, $idsPath;
print $cmd;
system $cmd;
